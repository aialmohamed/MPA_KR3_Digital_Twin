...
public class HomeInfo
{
    ...
    public string IsDockerRunning { get; private set; }
    ...
    public HomeInfo()
    {
        ...
        IsDockerRunning = CheckDockerRunning();
    ...
    private string CheckDockerRunning()
    {
        if (IsDockerInstalled.StartsWith("Docker is not installed"))
        {
            return "Cannot check if Docker is running because Docker is not installed.";
        }

        string result = CheckCommand("docker", "info", "Docker");
        if (result.StartsWith("Docker is not installed") || result.Contains("error during connect"))
        {
    ...
    private string CheckDockerImage(string imageName)
    {
        ...
        if (!IsDockerRunning.StartsWith("Docker is running"))
        {
            return $"Cannot check Docker image '{imageName}' because Docker is not running.";
        }
